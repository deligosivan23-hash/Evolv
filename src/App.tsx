...
export default function App() {
  ...
  // Per-habit independent timers — each habit has its own seconds/running/paused
  const [timers, setTimers] = useState<Record<string, { seconds: number; running: boolean; paused: boolean }>>({});
  ...
  // Tick ALL running timers every second independently
  useEffect(() => {
    const interval = setInterval(() => {
      setTimers(prev => {
        const hasRunning = Object.values(prev).some(t => t.running && !t.paused);
        if (!hasRunning) return prev;
        const next: typeof prev = {};
        for (const id in prev) {
          const t = prev[id];
          next[id] = (t.running && !t.paused) ? { ...t, seconds: t.seconds + 1 } : t;
        }
        return next;
      });
    }, 1000);
  ...
  const startTimer = (habitId: string) => {
    setTimers(prev => ({
      ...prev,
      // Resume from existing seconds if the habit was previously paused
      [habitId]: { seconds: prev[habitId]?.seconds || 0, running: true, paused: false }
    }));
  };
  const togglePause = (habitId: string) => {
    setTimers(prev => !prev[habitId] ? prev : {
      ...prev,
      [habitId]: { ...prev[habitId], paused: !prev[habitId].paused }
    });
  };
  const stopTimer = (habitId: string) => {
    // Freeze the timer and open the reflection modal for this habit
    setTimers(prev => !prev[habitId] ? prev : {
      ...prev,
      [habitId]: { ...prev[habitId], running: false, paused: false }
    });
  ...
  const saveReflection = () => {
    if (reflectionHabitId) {
      const seconds = timers[reflectionHabitId]?.seconds || 0;
      const info = todayHabits.find(h => h.habit.id === reflectionHabitId);
      const isRating = info?.goal.type === 'rating';
      logHabit(reflectionHabitId, isRating ? 0 : Math.floor(seconds / 60), reflectionText, isRating ? rating : undefined);
      // Remove this habit's timer entry after saving
...
// Dashboard
// ─────────────────────────────────────────────
function Dashboard({ todayHabits, standaloneTasks, onToggleTask, onDeleteTask, onAddTask, onAddGoal, timers, startTimer, togglePause, stopTimer, onCompleteChecklist, streak, dm, goals }: any) {
            ...
            {timedHabits.length === 0 ? (
              ...
              <div className="space-y-4">
                {timedHabits.map(({ habit, goal, hypothesis }: any) => {
                  ...
                  return <HabitCard key={habit.id} habit={habit} goal={goal} isActive={habitTimer.running || habitTimer.seconds > 0} isTimerPaused={habitTimer.paused} timerSeconds={habitTimer.seconds} onStart={() => startTimer(habit.id)} onPause={() => togglePause(habit.id)} onStop={() => stopTimer(habit.id)} mode="timer" adjustedTarget={adjustedTarget} dm={dm} />;
                })}
              </div>
            )}
          </div>
