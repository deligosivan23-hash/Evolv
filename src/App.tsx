...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState<boolean>(() => {
    return localStorage.getItem('evolv_dark') === 'true';
  });

  // Persist dark mode preference
  useEffect(() => {
    localStorage.setItem('evolv_dark', String(darkMode));
  }, [darkMode]);
  ...
  const streak = useMemo(() => {
    ...
    goals.forEach(g => {
      g.problems.forEach(p => {
        p.hypotheses.forEach(h => {
          h.habits.forEach(hab => {
            hab.logs.forEach(log => allLogs.push(log.date));
          });
        });
      });
    });
    ...
    while (true) {
      const hasLog = allLogs.some(d => isSameDay(parseISO(d), checkDate));
      if (!hasLog) break;
      count++;
  ...
  return (
    <div className={cn("min-h-screen font-sans selection:bg-[#E5E5E5] transition-colors duration-300", dm ? "bg-[#111111] text-white" : "bg-[#EBEBE9] text-[#1A1A1A]")}>
      {/* Sidebar / Navigation */}
      <nav className={cn("fixed bottom-0 left-0 right-0 md:top-0 md:bottom-0 md:w-20 border-t md:border-t-0 md:border-r z-50 flex md:flex-col items-center justify-around md:justify-center gap-8 py-4 md:py-8 transition-colors duration-300", dm ? "bg-[#1C1C1C] border-[#2A2A2A]" : "bg-white border-gray-100")}>
        <NavItem icon={<LayoutDashboard size={24} />} active={currentScreen === 'dashboard'} onClick={() => navigateTo('dashboard')} label="Home" darkMode={dm} />
        <NavItem icon={<Target size={24} />} active={currentScreen === 'goals' || currentScreen === 'goal-detail'} onClick={() => navigateTo('goals')} label="Goals" darkMode={dm} />
...
function NavItem({ icon, active, onClick, label, darkMode }: { icon: React.ReactNode, active: boolean, onClick: () => void, label: string, darkMode?: boolean }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        ...
        active
          ? (darkMode ? "text-white scale-110" : "text-[#1A1A1A] scale-110")
...
function Dashboard({ todayHabits, standaloneTasks, onToggleTask, onDeleteTask, onAddTask, activeHabitId, timerSeconds, isTimerRunning, isTimerPaused, startTimer, togglePause, stopTimer, onCompleteChecklist, streak, darkMode }: any) {
