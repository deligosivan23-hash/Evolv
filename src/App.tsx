...
import {
  ...
  Flame, Settings, Share2, Archive, ArchiveRestore, Pencil, Bell, Shield
} from 'lucide-react';
import {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setGoals(Array.isArray(p) ? p : (p.goals || []));
        setStandaloneTasks(Array.isArray(p) ? [] : (p.standaloneTasks || []));
      } catch {}
    }
  }, []);
  ...
  useEffect(() => {
    let interval: any;
    if (isTimerRunning && !isTimerPaused) interval = setInterval(() => setTimerSeconds(s => s + 1), 1000);
  ...
  const logHabit = (habitId: string, durationMinutes: number, reflection: string, habitRating?: number) => {
    setGoals(goals.map(g => {
      let updatedGoal = { ...g };
      let logsCount = 0, totalLogsNeeded = 0;
      updatedGoal.problems = g.problems.map(p => ({
        ...p, hypotheses: p.hypotheses.map(h => ({
          ...h, habits: h.habits.map(hab => {
            totalLogsNeeded += 1;
            if (hab.id === habitId) {
              const updated = { ...hab, logs: [...hab.logs, { id: crypto.randomUUID(), date: new Date().toISOString(), durationPerformed: durationMinutes, reflection, rating: habitRating }] };
              if (updated.logs.length > 0) logsCount += 1;
              return updated;
            }
  ...
  const streak = useMemo(() => {
    ...
    let count = 0; let cd = new Date();
    while (allLogs.some(d => isSameDay(parseISO(d), cd))) { count++; cd = subDays(cd, 1); }
    return count;
  }, [goals]);
  ...
  // Timer
  const startTimer = (habitId: string) => { setActiveHabitId(habitId); setTimerSeconds(0); setIsTimerRunning(true); setIsTimerPaused(false); };
  const togglePause = () => setIsTimerPaused(!isTimerPaused);
  const stopTimer = () => { setIsTimerRunning(false); setIsTimerPaused(false); setShowReflection(true); if (navigator.vibrate) navigator.vibrate(50); };
  const completeChecklistHabit = (habitId: string) => { setActiveHabitId(habitId); setTimerSeconds(0); setShowReflection(true); if (navigator.vibrate) navigator.vibrate(50); };
  const saveReflection = () => {
    if (activeHabitId) {
      const info = todayHabits.find(h => h.habit.id === activeHabitId);
      const isRating = info?.goal.type === 'rating';
      logHabit(activeHabitId, isRating ? 0 : Math.floor(timerSeconds / 60), reflectionText, isRating ? rating : undefined);
    }
    setShowReflection(false); setReflectionText(''); setRating(5); setActiveHabitId(null); setTimerSeconds(0);
  };
...
// Dashboard
// ─────────────────────────────────────────────
function Dashboard({ todayHabits, standaloneTasks, onToggleTask, onDeleteTask, onAddTask, onAddGoal, activeHabitId, timerSeconds, isTimerRunning, isTimerPaused, startTimer, togglePause, stopTimer, onCompleteChecklist, streak, dm, goals }: any) {
            ...
            {timedHabits.length === 0 ? (
              ...
              <div className="space-y-4">
                {timedHabits.map(({ habit, goal, hypothesis }: any) => {
                  ...
                  let totalActual = 0;
                  goal.problems.forEach((p: any) => p.hypotheses.forEach((h: any) => h.habits.forEach((hab: any) => hab.logs.forEach((l: any) => { totalActual += l.durationPerformed; }))));
                  const adjustedTarget = Math.max(0, Math.round((goal.targetValue - totalActual) / daysRemaining));
                  return <HabitCard key={habit.id} habit={habit} goal={goal} isActive={activeHabitId === habit.id} isTimerPaused={isTimerPaused} timerSeconds={timerSeconds} onStart={() => startTimer(habit.id)} onPause={togglePause} onStop={stopTimer} mode="timer" adjustedTarget={adjustedTarget} dm={dm} />;
                })}
              </div>
            )}
          </div>
...
// HabitCard
// ─────────────────────────────────────────────
function HabitCard({ habit, goal, isActive, isTimerPaused, timerSeconds, onStart, onPause, onStop, onComplete, mode = 'timer', adjustedTarget, dm }: any) {
  ...
  return (
    <div className={cn('group relative border rounded-2xl p-6 transition-all duration-300', dm ? 'bg-[#1C1C1C]' : 'bg-white', isActive ? 'ring-2 shadow-xl scale-[1.01]' : (dm ? 'border-[#2A2A2A]' : 'border-gray-100 hover:border-gray-200'))} style={isActive ? { borderColor: BRAND, '--tw-ring-color': BRAND } as any : {}}>
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {habitStreak > 0 && <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono text-white" style={{ background: BRAND }}><Flame size={9} /> {habitStreak}</span>}
          </div>
          ...
          <div className="flex items-center gap-3 text-xs text-gray-400 flex-wrap">
            ...
            {!isChecklist && (
              <span className={cn('flex items-center gap-1 px-2 py-0.5 rounded-md', showAdjusted ? 'bg-amber-50 text-amber-700 border border-amber-100' : '')}>
                <Timer size={11} />{showAdjusted ? (<><span className="line-through opacity-50">{habit.duration}m</span><span className="font-bold">{adjustedTarget}m</span><span className="text-[8px]">catch-up</span></>) : `${habit.duration}m`}
              </span>
            )}
        ...
        <div className="flex flex-col items-end gap-3 shrink-0">
          {isChecklist ? (
            ...
            <div className="flex items-center gap-2">
              <span className="text-xl font-mono tabular-nums">{fmt(timerSeconds)}</span>
      ...
      {!isChecklist && isActive && (
        <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min((timerSeconds / (habit.duration * 60)) * 100, 100)}%` }} className="absolute bottom-0 left-0 h-1 rounded-b-2xl" style={{ background: BRAND }} />
      )}
    </div>
  );
}
...
// GoalsList
// ─────────────────────────────────────────────
function GoalsList({ goals, onSelectGoal, onAddGoal, dm }: any) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-12">
      <header className="flex items-center justify-between">
        ...
        <button onClick={onAddGoal} style={{ background: BRAND }} className="flex items-center gap-2 text-white px-6 py-3 rounded-full text-sm font-medium hover:opacity-90 transition-opacity">
...
// GoalDetail
// ─────────────────────────────────────────────
function GoalDetail({ goal, onBack, onAddProblem, onSelectProblem, onDeleteProblem, onDeleteGoal, onSetProgress, dm }: any) {
  ...
  const metrics = useMemo(() => {
    ...
    return { totalActual, linearTarget, deficit: linearTarget - totalActual, adjustedDaily: daysRemaining > 0 ? (target - totalActual) / daysRemaining : 0, originalDaily: target / totalDays, daysRemaining, percentComplete: (totalActual / target) * 100, linearPercent: (linearTarget / target) * 100 };
  }, [goal]);
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
      ...
      <header className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-light tracking-tight">{goal.title}</h1>
        </div>
        ...
        <div className={cn('space-y-4 p-6 rounded-3xl border', dm ? 'bg-[#1C1C1C] border-[#2A2A2A]' : 'bg-gray-50 border-gray-100')}>
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-2xl font-light">{Math.round(metrics.totalActual)} <span className="text-sm text-gray-400">/ {goal.targetValue} {goal.targetUnit}</span></p>
            </div>
            <div className="text-right flex flex-col items-end gap-2">
              <p className={cn('text-sm font-medium', metrics.deficit > 0 ? 'text-red-400' : 'text-green-500')}>
                {metrics.deficit > 0 ? `Behind by ${Math.round(metrics.deficit)}` : `Ahead by ${Math.round(Math.abs(metrics.deficit))}`} {goal.targetUnit}
              </p>
              {goal.type === 'rating' && (
          ...
          <div className={cn('relative h-2.5 rounded-full overflow-hidden', dm ? 'bg-[#2A2A2A]' : 'bg-gray-200')}>
            <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(100, metrics.linearPercent)}%` }} className="absolute top-0 left-0 h-full opacity-30 z-0 rounded-full" style={{ background: goal.color }} />
          ...
          <div className="grid grid-cols-3 gap-3">
            {[['Days Left', metrics.daysRemaining], ['Orig. Daily', `${Math.round(metrics.originalDaily)} ${goal.targetUnit}`], ['Adj. Daily', `${Math.max(0, Math.round(metrics.adjustedDaily))} ${goal.targetUnit}`]].map(([label, val]) => (
          ...
          {goal.problems.map((p: Problem) => (
            <div key={p.id} className={cn('flex items-center justify-between border rounded-2xl p-5 group transition-all', dm ? 'bg-[#1C1C1C] border-[#2A2A2A] hover:border-[#7A5230]' : 'bg-white border-gray-100 hover:border-[#7A5230]')}>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => onDeleteProblem(p.id)} className="text-gray-300 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14} /></button>
...
// ProblemDetail
// ─────────────────────────────────────────────
function ProblemDetail({ goal, problem, onBack, onAddHypothesis, onSelectHypothesis, onDeleteHypothesis, dm }: any) {
          ...
          {problem.hypotheses.map((h: Hypothesis, idx: number) => (
            <div key={h.id} className={cn('border rounded-2xl p-7 group transition-all', dm ? 'bg-[#1C1C1C] border-[#2A2A2A] hover:border-[#7A5230]' : 'bg-white border-gray-100 hover:border-[#7A5230]')}>
              <div className="flex items-start justify-between">
                <button onClick={() => onSelectHypothesis(h.id)} className="text-left flex-1 space-y-3">
...
// HypothesisDetail
// ─────────────────────────────────────────────
function HypothesisDetail({ goal, problem, hypothesis, onBack, onAddHabit, onDeleteHabit, onUpdateHabit, onArchiveHabit, onAddJournalNote, goalType, dm }: any) {
...
// Settings Screen
// ─────────────────────────────────────────────
function SettingsScreen({ dm, onToggleDark, onExport, onClearData }: any) {
...
// Analytics
// ─────────────────────────────────────────────
function Analytics({ goals, onExport, dm }: any) {
  ...
  const stats = useMemo(() => {
    ...
    if (timeRange === 'day') {
      return goals.flatMap((g: Goal) => g.problems.flatMap((p: Problem) => p.hypotheses.flatMap((h: Hypothesis) => h.habits.filter((hab: Habit) => !hab.archived && hab.days.includes(now.getDay())).map((hab: Habit) => {
        const log = hab.logs.find(l => isSameDay(parseISO(l.date), now));
        return { name: hab.title.slice(0, 10), actual: log?.durationPerformed || 0, target: hab.duration, deficit: Math.max(0, hab.duration - (log?.durationPerformed || 0)), avgRating: log?.rating || 0 };
      }))));
    }
    if (timeRange === 'month') {
  ...
  const dailyRecords = useMemo(() => {
    ...
    goals.forEach((g: Goal) => g.problems.forEach((p: Problem) => p.hypotheses.forEach((h: Hypothesis) => h.habits.forEach((hab: Habit) => hab.logs.forEach(log => {
      ...
      if (inc) records.push({ id: log.id, date: log.date, habitTitle: hab.title, goalTitle: g.title, duration: log.durationPerformed, target: hab.duration, rating: log.rating, reflection: log.reflection, color: g.color });
    })))));
        ...
        <div className="lg:col-span-2">
          <div className={cn('border rounded-2xl p-7 space-y-5', dm ? 'bg-[#1C1C1C] border-[#2A2A2A]' : 'bg-white border-gray-100')}>
            ...
            {dailyRecords.length === 0 ? <p className="text-gray-400 text-sm italic text-center py-8">No records found.</p> : (
              <div className="space-y-4">
                {dailyRecords.map(r => (
                  <div key={r.id} className={cn('border-b last:border-0 pb-4 last:pb-0 space-y-1.5', dm ? 'border-[#2A2A2A]' : 'border-gray-50')}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
...
function GoalFormModal({ onClose, onSubmit, dm }: any) {
  ...
  const [formData, setFormData] = useState({ title: '', type: 'time' as GoalType, deadline: format(addDays(new Date(), 30), 'yyyy-MM-dd'), targetValue: 0, targetUnit: '', color: `linear-gradient(135deg, ${BRAND} 0%, #4A2C18 100%)` });
  const colors = [
    { v: `linear-gradient(135deg, ${BRAND} 0%, #4A2C18 100%)` },
    { v: 'linear-gradient(135deg, #1A1A1A 0%, #4A4A4A 100%)' },
    { v: 'linear-gradient(135deg, #3B82F6 0%, #1D4ED8 100%)' },
    { v: 'linear-gradient(135deg, #EF4444 0%, #B91C1C 100%)' },
    { v: 'linear-gradient(135deg, #10B981 0%, #047857 100%)' },
    { v: 'linear-gradient(135deg, #F59E0B 0%, #B45309 100%)' },
    { v: 'linear-gradient(135deg, #8B5CF6 0%, #6D28D9 100%)' },
  ];
  const inputCls = cn('w-full text-2xl font-light border-b-2 focus:outline-none py-4 transition-all', dm ? 'bg-transparent text-white border-[#2A2A2A] focus:border-[#7A5230]' : 'border-gray-100 focus:border-[#7A5230]');
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={cn('w-full max-w-lg rounded-3xl p-10 space-y-8 relative', dm ? 'bg-[#1C1C1C] text-white' : 'bg-white')}>
        ...
        <div><span className="text-[10px] font-mono uppercase tracking-widest text-gray-400">Step {step} of 4</span>
          <h2 className="text-2xl font-light mt-1">{['', "What's the outcome?", "How will you track it?", "When is the deadline?", "What's the target?"][step]}</h2></div>
        <div className="min-h-[150px]">
          {step === 1 && <div className="space-y-6">
            <input autoFocus type="text" placeholder="e.g. Finish 10 chapters of Calculus" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className={inputCls} />
            <div className="flex flex-wrap gap-3">{colors.map(c => <button key={c.v} onClick={() => setFormData({ ...formData, color: c.v })} className={cn('w-10 h-10 rounded-xl shadow-sm transition-all', formData.color === c.v && 'ring-2 ring-offset-2 scale-110')} style={{ background: c.v, ringColor: BRAND }} />)}</div>
          </div>}
          {step === 2 && <div className="grid gap-3">
            {(['time', 'rating'] as GoalType[]).map(t => (
              <button key={t} onClick={() => setFormData({ ...formData, type: t })} className={cn('p-5 rounded-2xl border text-left transition-all', formData.type === t ? 'border-[#7A5230] bg-[#7A5230]/5' : dm ? 'border-[#2A2A2A] hover:border-[#3A3A3A]' : 'border-gray-100 hover:border-gray-200')}>
                <div className="flex items-center gap-3 mb-1"><Timer size={18} /><span className="font-medium capitalize">{t}-based Tracking</span></div>
                <p className="text-xs text-gray-400">{t === 'time' ? 'Track by time spent. Best for study, practice, deep work.' : 'Rate 1-10 at session end. Best for meditation, mood, quality goals.'}</p>
              </button>
            ))}
          </div>}
          {step === 3 && (
            <div className="space-y-3">
              <p className="text-sm text-gray-400">Select your target deadline</p>
              <input autoFocus type="date" value={formData.deadline} onChange={e => setFormData({ ...formData, deadline: e.target.value })} className={cn('w-full rounded-2xl px-5 py-4 border focus:outline-none transition-all text-base', dm ? 'bg-[#252525] border-[#2A2A2A] text-white focus:border-[#7A5230]' : 'bg-gray-50 border-transparent focus:border-[#7A5230]')} />
            </div>
          )}
          {step === 4 && <div className="flex gap-4">
            <input autoFocus type="number" placeholder="0" value={formData.targetValue || ''} onChange={e => setFormData({ ...formData, targetValue: Number(e.target.value) })} className={cn(inputCls, 'w-1/3')} />
            <input type="text" placeholder="unit (hours, chapters…)" value={formData.targetUnit} onChange={e => setFormData({ ...formData, targetUnit: e.target.value })} className={cn(inputCls, 'w-2/3')} />
          </div>}
        </div>
        <div className="flex items-center justify-between pt-2">
          {step > 1 ? <button onClick={() => setStep(step - 1)} className="text-gray-400 font-medium">Back</button> : <div />}
          {step < 4 ? (
            <button disabled={step === 1 && !formData.title} onClick={() => setStep(step + 1)} style={{ background: BRAND }} className="text-white px-8 py-3 rounded-full font-medium disabled:opacity-30 hover:opacity-90 transition-opacity">Next</button>
          ) : (
            <button disabled={!formData.targetUnit || !formData.targetValue} onClick={() => onSubmit(formData)} style={{ background: BRAND }} className="text-white px-8 py-3 rounded-full font-medium disabled:opacity-30 hover:opacity-90 transition-opacity">Create Goal</button>
          )}
        </div>
      </motion.div>
    </div>
...
function ReflectionModal({ text, setText, rating, setRating, isRatingType, onSave, duration, dm }: any) {
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[110] flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={cn('w-full max-w-lg rounded-3xl p-10 space-y-8 text-center', dm ? 'bg-[#1C1C1C] text-white' : 'bg-white')}>
        ...
        <div><h2 className="text-2xl font-light">Session Complete</h2><p className="text-gray-400 mt-1">You tracked {Math.floor(duration / 60)}m {duration % 60}s.</p></div>
        <div className="space-y-5 text-left">
          ...
          <div className="space-y-2"><label className="text-[10px] font-mono uppercase tracking-widest text-gray-400">What did you learn?</label>
            <textarea autoFocus placeholder="Capture your insights..." value={text} onChange={e => setText(e.target.value)} className={cn('w-full border-none rounded-2xl px-5 py-5 focus:ring-1 focus:outline-none min-h-[130px] resize-none', dm ? 'bg-[#252525] text-white placeholder-gray-600 focus:ring-[#7A5230]' : 'bg-gray-50 focus:ring-[#7A5230]')} />
          </div>
        </div>
        <button onClick={onSave} style={{ background: BRAND }} className="w-full text-white py-4 rounded-full font-medium hover:opacity-90 transition-opacity">Save Reflection</button>
      </motion.div>
    </div>
  );
}
